...
class Backtest {
    constructor() {
        ...
        Promise.all([usaAllocationQuery, euAllocationQuery, sp500MonthlyQuery, eafeMonthlyQuery]).spread(function(usaAllocations, euAllocations, sp500Monthly, eafeMonthly) {
            ...
            for (let i = 0; i < euAllocations.length; i++) {
                ...
                const eafeExpectedReturns = eafeRegressionResults.equation[0] * euLatestAllocation + eafeRegressionResults.equation[1];
                const sp500ExpectedReturns = sp500RegressionResults.equation[0] * usaLatestAllocation + sp500RegressionResults.equation[1];
                const sp500Price = parseFloat(sp500MonthlyFiltered[i].value);
                const eafePrice = parseFloat(eafeMonthlyFiltered[i].value);

                console.log('SP500 vs EAFE expected returns:', sp500ExpectedReturns, eafeExpectedReturns, euAllocations[i].date);
            ...
            const results = [activePortfolio, eafePortfolio, sp500Portfolio].map(function(portfolio) {
                ...
                const sp500Value = parseFloat(sp500MonthlyFiltered[sp500MonthlyFiltered.length - 1].value) * portfolio.sp500Shares;
