(function() {
  ...
  angular.module('app').directive('unemploymentChart', function($rootScope) {
    return {
      restrict: 'A',
      scope: {
        unemploymentData: '&',
      ...
      link: function($scope, $elem) {
        ...
        const unemploymentData = $scope
          .unemploymentData()
          .map(i => [new Date(i.date).getTime(), parseFloat((parseFloat(i.percentage) * 100).toFixed(1))]);

        const unemploymentMAData = unemploymentData.slice(12).map(function(item, index) {
          const movingAverage =
            unemploymentData.slice(index, index + 12).reduce(function(total, item) {
              return total + item[1];
            }, 0) / 12;
